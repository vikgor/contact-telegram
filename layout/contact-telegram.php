...
if ($sendToTelegram) {
	$resultAlert = '<div class="alert alert-success">' .$L->get('thank-you-for-contact'). '</div>' ."\r\n";
	echo $resultAlert;
	return true;
} else {
	$resultAlert = '<div class="alert alert-danger">' .$L->get('an-error-occurred-while-sending'). '</div>' ."\r\n";
	echo $resultAlert;
}
}
...
<form method="post" action="<?php echo '.' . DS . $page->slug(); ?>" class="contact-telegram">
